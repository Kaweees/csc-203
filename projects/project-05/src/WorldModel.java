...
public final class WorldModel {
  ...
  private static final int GRAVE_NUM_PROPERTIES = 0;
  ...
  private static final int ZOMBIE_ANIMATION_PERIOD = 0;
  private static final int ZOMBIE_ACTION_PERIOD = 1;
  private static final int ZOMBIE_NUM_PROPERTIES = 0;
  ...
  public void parseDude(String[] properties, Point pt, String id, ImageStore imageStore) {
    if (properties.length == DUDE_NUM_PROPERTIES) {
      AStarPathingVirtual strategy = new AStarPathingVirtual();
  ...
  private void parseZombie(String[] properties, Point pt, String id, ImageStore imageStore) {
    if (properties.length == ZOMBIE_NUM_PROPERTIES) {
      AStarPathingVirtual strategy = new AStarPathingVirtual();
      Entity entity = new WorldModel().createZombie(id, pt, Double.parseDouble(properties[ZOMBIE_ACTION_PERIOD]),
          Double.parseDouble(properties[ZOMBIE_ANIMATION_PERIOD]), imageStore.getImageList(ZOMBIE_KEY), strategy);
      tryAddEntity(entity);
    } else {
      throw new IllegalArgumentException(
          String.format("%s requires %d properties when parsing", ZOMBIE_KEY, ZOMBIE_NUM_PROPERTIES));
    }
  }
  ...
  private void parseGrave(String[] properties, Point pt, String id, ImageStore imageStore) {
    if (properties.length == GRAVE_NUM_PROPERTIES) {
      Entity entity = new WorldModel().createGrave(id, pt, imageStore.getImageList(GRAVE_KEY));
      tryAddEntity(entity);
    } else {
      throw new IllegalArgumentException(
          String.format("%s requires %d properties when parsing", GRAVE_KEY, GRAVE_NUM_PROPERTIES));
    }
  }
  ...
  public void parseEntity(String line, ImageStore imageStore) {
    ...
    if (properties.length >= ENTITY_NUM_PROPERTIES) {
      ...
      case ZOMBIE_KEY -> this.parseZombie(properties, pt, id, imageStore);
      case FAIRY_KEY -> this.parseFairy(properties, pt, id, imageStore);
      case HOUSE_KEY -> this.parseHouse(properties, pt, id, imageStore);
      case GRAVE_KEY -> this.parseGrave(properties, pt, id, imageStore);
  ...
  public Entity createGrave(String id, Point position, List<PImage> images) {
    return new Grave(id, position, images);
  }
  ...
  public Entity createZombie(String id, Point position, double actionPeriod, double animationPeriod, List<PImage> images,
      PathingStrategy strategy) {
    return new Zombie(id, position, images, animationPeriod, actionPeriod, strategy);
  }
  ...
  public String getTileType(Point pressed) {
    return this.getBackgroundCell(pressed).getId();
  }
}
